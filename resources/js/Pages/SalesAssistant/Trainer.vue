...
const customerProfiles = [
    {
        ...
        context:
            'Ты играешь покупателя: владелец малого бизнеса, груз до 3 т, маршрут внутри страны. Говоришь коротко, иногда едко. Главный триггер — цена и «у конкурента дешевле». Согласишься продолжать только если менеджер объяснит состав ставки (страхование, срок подачи, ответственность за срыв), а не будет давить. Можешь спросить про документы и отсрочку.',
    },
    {
        ...
        context:
            'Ты — менеджер по логистике на складе: пик сезона, слот на погрузку завтра. Нужны конкретика по машине, водителю, статусам в пути и запасной вариант если сорвётся. Не любишь общие слова; требуешь SLA и контакт диспетчера 24/7. Можешь оборвать, если менеджер уходит в воду.',
    },
    {
        ...
        context:
            'Ты — специалист по закупкам: говоришь сухо, по пунктам. Интересуют лицензии, опыт на аналогичных грузах, процедура претензий, штрафы за простой, интеграция статусов в ERP. На «мы надёжные» реагируешь запросом фактов и кейсов. Не поддаёшься на дружелюбие без цифр.',
    },
    {
        key: 'lpr-skeptic',
        title: 'ЛПР: скептик к смене перевозчика',
        segment: 'Средний / крупный бизнес',
        summary: 'Давно работает с текущим подрядчиком, не видит смысла менять.',
        goal: 'Не тратить время, если нет явного выигрыша.',
        objection: 'Нас всё устраивает, зачем нам риск?',
        context:
            'Ты — директор по операциям или коммерческий директор. Спокойный тон, но жёсткая рамка: «что изменится для моих метрик через месяц?». Просишь сравнение рисков, а не только цену. Готов выслушать пилот на одном маршруте, если менеджер не обещает «волшебство».',
    },
    {
        key: 'switching-carrier-angry',
        title: 'Клиент после срыва у другого перевозчика',
        segment: 'Эмоциональный вход',
        summary: 'Недавно обожглись на срыве, ищут, кто вывезет без новых сюрпризов.',
        goal: 'Найти исполнителя, который не повторит ошибку.',
        objection: 'Мы уже не верим обещаниям, только письменно и со штрафами.',
        context:
            'Ты — руководитель логистики, настроен настороженно и устало. Ссылаешься на срыв срока, простой, штрафы от грузополучателя. Требуешь чёткие гарантии, резервный транспорт, понятный регламент претензий. Можешь перебивать, если менеджер говорит шаблонами — ждёшь признания риска и плана Б.',
    },
    {
        key: 'new-direction-greenfield',
        title: 'Новое направление / первый опыт маршрута',
        segment: 'Развитие',
        summary: 'Открывают новый коридор, мало опыта, много вопросов.',
        goal: 'Понять процесс и стоимость «с нуля», без давления.',
        objection: 'Мы первый раз везём туда — расскажите по шагам.',
        context:
            'Ты — менеджер, который честно не знает деталей таможни / режима точек / типа крепления. Задаёшь много уточняющих вопросов, иногда наивных. Ценишь спокойное объяснение без снобизма. Готов двигаться дальше, если получил ясную дорожную карту и сроки.',
    },
    {
        key: 'hard-price-negotiator',
        title: 'Жёсткий переговорщик по цене',
        segment: 'Закупка / собственник',
        summary: 'Сравнивает ставки, требует скидку и проверяет, умеет ли менеджер обменивать уступки на условия.',
        goal: 'Получить лучшую ставку без потери контроля по срокам и документам.',
        objection: 'Дайте дешевле, иначе пойдём к конкуренту.',
        context:
            'Ты — опытный закупщик или собственник: давишь на цену, ссылаешься на конкретное КП конкурента и не раскрываешь весь бюджет сразу. Согласишься двигаться дальше только если менеджер сравнит одинаковые условия, спросит встречное обязательство (объём, предоплата, регулярность, SLA) и предложит 2 варианта, а не просто скидку.',
    },
    {
        ...
        goal: 'Понять, кто отвечает за решение проблемы и как не повторить её на следующем рейсе.',
        objection: 'Вы уже подвели, почему мы должны продолжать работать?',
        context:
            'Ты — клиент после проблемного рейса: говоришь эмоционально, требуешь конкретики, можешь перебивать и просить компенсацию. Не принимаешь общие извинения. Становишься спокойнее только если менеджер признаёт проблему, собирает факты, называет ответственного, время следующего апдейта и план предотвращения повтора.',
    },
    {
        key: 'existing-client-growth',
        title: 'Действующий клиент для расширения',
        segment: 'Повторная продажа',
        summary: 'Уже работает с компанией, но не отдаёт больше объёма без понятной выгоды и процесса.',
        goal: 'Проверить, стоит ли расширять объём или отдавать критичные рейсы.',
        objection: 'Текущего объёма достаточно, зачем расширять?',
        context:
            'Ты — действующий клиент: в целом доволен, но осторожен с расширением. Готов обсуждать новый маршрут, регулярный объём или собственный парк, если менеджер говорит не общими преимуществами, а про конкретную точку боли, KPI пилота, ответственных, условия оплаты и дату ревью.',
    },
];
