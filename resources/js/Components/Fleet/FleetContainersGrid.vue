  ...
  <div ref="gridSection" class="flex min-h-0 min-w-0 flex-1 flex-col gap-2">
    <div class="flex shrink-0 items-center justify-between gap-2">
      ...
      <div class="text-xs text-zinc-500 dark:text-zinc-400">
        Двойной клик или кнопка «Изменить» открывает карточку контейнера
      </div>
    </div>
...
const filterModelStorageKey = computed(() => agGridFilterModelStorageKey('fleet_containers', props.userId));
...
const columnStorageKey = computed(() => `fleet_containers_grid_columns_v3_${props.userId}`);
...
const gridOptions = {
  ...
  doesExternalFilterPass: (node) => {
    ...
    const haystack = [data.owner_name, data.container_number, data.size_label, data.type_label, data.size_code, data.container_type]
      .filter(Boolean)
      .join(' ')
      .toLowerCase();
...
function buildBaseContainerColumnDefs() {
  return [
    { field: 'id', headerName: 'ID', width: 56, minWidth: 48, maxWidth: 72, filter: false, floatingFilter: false, suppressHeaderFilterButton: true, getQuickFilterText: () => '' },
    {
      ...
      cellRenderer: editContainerCellRenderer,
    },
    { field: 'owner_name', headerName: 'Владелец', flex: 1, minWidth: 160, floatingFilter: true },
    { field: 'container_number', headerName: 'Номер', width: 140 },
    { field: 'size_label', headerName: 'Размер', width: 100 },
    { field: 'type_label', headerName: 'Тип', width: 120 },
    { field: 'documents_count', headerName: 'Док.', width: 72, filter: 'agNumberColumnFilter' },
  ];
}

function editContainerCellRenderer(params) {
...
const columnDefs = computed(() => {
  ...
  const raw = buildBaseContainerColumnDefs();

  if (!saved?.length) {
    return raw;
  }

  const fields = raw.map((c) => c.field ?? c.colId);
  const { orderedFields, byColId } = buildLayoutIndex(fields, saved);
  const byField = new Map(raw.map((d) => [d.field ?? d.colId, d]));
