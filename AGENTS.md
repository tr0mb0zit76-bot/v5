...
- Таблица точек маршрута (`route_point_row_*`): при `printParty=customer` и ≥2 плечах `PrintFormCustomerRoutePointFilter` скрывает стыки плеч (перегрузка под смену перевозчика); сортировка `(leg.sequence, point.sequence)`.
