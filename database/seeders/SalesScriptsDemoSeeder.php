    ...
    public function run(): void
    {
        ...
        $this->seedScript(
            ...
            nodes: [
                ...
                ['client_key' => 'status_probe', 'kind' => SalesScriptNodeKind::Branch, 'body' => 'Спросить: «На каком этапе сейчас вопрос: ещё выбираете, ждёте внутреннее согласование, уже выбрали подрядчика или отложили саму перевозку?» Зафиксируйте главный блокер — {decision_criteria}, текущего подрядчика — {current_provider}, дедлайн решения — {decision_deadline}.', 'hint' => 'Главный результат шага — классифицировать паузу: нет потребности, нет ЛПР, цена, конкурент, нет данных, не время.', 'sort_order' => 20, 'tags' => ['статус', 'блокер'], 'capture_field_codes' => ['decision_criteria', 'current_provider', 'decision_deadline']],
        ...
        $this->seedScript(
            ...
            nodes: [
                ...
                ['client_key' => 'price_diagnosis', 'kind' => SalesScriptNodeKind::Branch, 'body' => 'Диагностика давления: клиент сравнивает с конкурентом, бюджетом, прошлой ставкой или просто проверяет уступку? Спросить: «С чем сравниваем: конкретное КП, целевой бюджет, предыдущая перевозка или внутренний лимит?» Зафиксируйте критерий — {decision_criteria}, конкурента — {current_provider}, бюджет — {budget_window}.', 'hint' => 'Разные причины «дорого» требуют разных ответов. Не лечите все скидкой.', 'sort_order' => 20, 'tags' => ['диагностика цены'], 'capture_field_codes' => ['decision_criteria', 'current_provider', 'budget_window']],
        ...
        $this->seedScript(
            ...
            nodes: [
                ...
                ['client_key' => 'fact_check', 'kind' => SalesScriptNodeKind::Branch, 'body' => 'Собрать факты: что произошло, где груз/машина, какой ущерб или риск, кто на стороне клиента ждёт информацию, какой дедлайн критичен. Заполните план восстановления — {service_recovery_plan} и дедлайн решения — {decision_deadline}.', 'hint' => 'Отделяйте факты от эмоций. Не обещайте невозможное до проверки.', 'sort_order' => 20, 'tags' => ['факты', 'SLA'], 'capture_field_codes' => ['service_recovery_plan', 'decision_deadline']],
        ...
        $this->seedScript(
            ...
            nodes: [
                ...
                ['client_key' => 'growth_probe', 'kind' => SalesScriptNodeKind::Branch, 'body' => 'Спросить: какие направления растут — {routes}; какой прогноз объёма — {volume_forecast}; где текущая схема не справляется; кто принимает решение по расширению — {decision_maker}; какие критерии успеха — {decision_criteria}.', 'hint' => 'Ищите конкретную точку расширения, а не “дайте нам всё”.', 'sort_order' => 20, 'tags' => ['диагностика роста'], 'capture_field_codes' => ['routes', 'volume_forecast', 'decision_maker', 'decision_criteria']],
        ...
        $this->seedScript(
            ...
            nodes: [
                ...
                ['client_key' => 'trainer_branch', 'kind' => SalesScriptNodeKind::Branch, 'body' => 'Выберите реакцию клиента: «дороже конкурента», «нет бюджета», «дайте скидку без условий», «готов обсудить объём», «нужно согласовать». Соберите критерии — {decision_criteria}, конкурента — {current_provider}, объём — {volume_forecast}.', 'hint' => 'Тренажёр должен заставлять менеджера искать обмен, а не уступать в одни ворота.', 'sort_order' => 20, 'tags' => ['ветвление', 'тренажёр'], 'capture_field_codes' => ['decision_criteria', 'current_provider', 'volume_forecast']],
        ...
        $this->seedScript(
            ...
            nodes: [
                ...
                ['client_key' => 'trainer_branch', 'kind' => SalesScriptNodeKind::Branch, 'body' => 'Выберите реакцию клиента: задержка, документы, требование компенсации, угроза уйти к конкуренту, пауза по будущим заказам. Заполните план восстановления — {service_recovery_plan} и нужные документы — {required_documents}.', 'hint' => 'В конфликте каждая ветка должна закончиться временем следующего апдейта.', 'sort_order' => 20, 'tags' => ['ветвление', 'претензия'], 'capture_field_codes' => ['service_recovery_plan', 'required_documents']],
