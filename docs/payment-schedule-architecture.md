...
1. **События документов / погрузки / выгрузки** (`basis` = `fttn`, `fttn_receipt`, `ottn`, `loading`, `unloading`):
   ...
   - **наличная форма оплаты** (`payment_form` = `cash`):
     ...
     - у **перевозчика/подрядчика** базисы **`ottn`** / **`fttn_receipt`** → **`waybill`** (дата прикрепления ТСД/ТН);
     - у **заказчика** **`ottn`** / **`fttn_receipt`**: полный чек-лист + обе даты оригиналов (заявка и закрывающие), старт — max из двух;
...
| OTTN / FTTN_RECEIPT (по оригиналам) | 1) обязательный чек-лист стороны полностью закрыт; 2) проставлены **обе** даты «оригиналы заявки» и «оригиналы закрывающих»; старт отсчёта — **более поздняя** из двух + сдвиг. Базисы `ottn` и `fttn_receipt` ведут себя одинаково. |
