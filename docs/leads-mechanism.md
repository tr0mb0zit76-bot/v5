...
| **Сбор деталей** (квалификация) | Уточняем запрос, клиента, условия |
...
- Раздел **Лиды** в меню → таблица с фильтрами и поиском.
- **Двойной клик** по строке — карточка лида.
- **Добавить** — новый лид.

Видимость: по роли пользователь видит **только свои** лиды или **все** (настраивается в роли).
...
На списке лидов может отображаться блок коучинга по **закрытым** сделкам за период (win rate, типичные ошибки). Это аналитика для обучения, не очередь текущих действий.
...
| **Маршрут** | Точки погрузки и выгрузки: адрес, дата, контакты на точке |
...
Вкладки **Коммерческое**, **Документы** и часть действий доступны только у **уже сохранённого** лида.
...
| Срок | Когда нужна перевозка / решение |
...
- **Следующий контакт** — дата/время планового касания; при просрочке может сработать напоминание.
...
- Точки с типом (погрузка / выгрузка / промежуточная), адресом, плановой датой, контактом на точке.
...
2. Создаётся **новый заказ** (или открывается уже существующий, если конвертация была раньше).
3. **Переносится:** контрагент, даты, примечания, маршрут, груз, цена и валюта, финансовые поля где применимо.
4. Статус лида → **Выигран**; открывается **мастер заказа**.
...
| --- | --- |
| **Задачи** | Задачи с привязкой к лиду; смена статуса задачи может влиять на статус лида (см. [tasks-user-guide.md](./tasks-user-guide.md)) |
...
1. Заполнить **Маршрут** и **Груз** (статус поднимется до «Просчёт»).
2. Указать **цену** на «Финансы» или «Основное».
...
*Актуально на июнь 2026. При изменении интерфейса обновляйте этот файл и краткую инструкцию.*
