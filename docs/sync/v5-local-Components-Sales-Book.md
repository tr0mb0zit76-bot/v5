...
| --- | --- | --- |
| Лиды — инструкция | prod id=19 / локально по title «Лиды — инструкция для пользователя» | `php scripts/sync-leads-sales-book-article.php` |
...
Источник правды для лидов: `docs/lead-user-guide.md` (инструкция для менеджера) + `docs/leads-mechanism.md` (полный регламент). После правок в git — скрипт на каждой среде (local/prod), т.к. контент статьи в БД.
