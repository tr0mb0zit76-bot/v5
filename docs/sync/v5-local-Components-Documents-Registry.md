...
- **Реестр** (`/documents`) — ag-Grid по заказам, колонки по типам документов + даты «Оригиналы заказчика/перевозчика». Пустая ячейка → **«не требуется»**, если слот не в чек-листе (`DocumentRegistryGridColumnApplicabilityResolver`, те же правила, что в мастере).
