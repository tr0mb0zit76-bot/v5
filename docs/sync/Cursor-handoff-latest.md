...
| Блок | Статус |
| --- | --- |
| Очистка факта в мастере реально снимает `route_points.actual_date` | ✅ |
| MCP/сервис: `clear` / пустая date снимает факт | ✅ |
| После проставленной **выгрузки** менять/снять может только admin/supervisor | ✅ UI + wizard + route service |
| PHPUnit authorization + clear + closing notify | ✅ |
| **Commit + push** `74e375ec` · **прод** pull + `npm run build` | → в этом деплое |

**Следующий шаг:** smoke — менеджер не меняет закрытую выгрузку; руководитель меняет/чистит.

---

**Обновлено (архив):** 2026-09-21 12:45 · **Ветка:** `master` · **HEAD:** `3c85bec2` · **тема:** CMR/ТН на нескольких плечах
...
| Блок | Статус |
| --- | --- |
| Сумма межфирменной = `customer_rate × 0.95`; оплата/НДС с клиентской стороны | ✅ |
...
**Обновлено (архив):** 2026-09-18 10:25 · **Ветка:** `master` · **HEAD:** `f21bf326` · **тема:** harden УУ matching + ссылка из акта на строку выписки
...
| DOCX анализ: `docs/DDS-CFO-analiz-2026-09-17.docx` | ✅ в git |
| **Commit + push** `82c702dd` | ✅ |
| **Прод** `git pull` + `optimize:clear` | ✅ smoke: net +704707, ДЗ 676230, КЗ 794265 |
...
**Следующий шаг (на другом ПК):** `ЗАБРАТЬ` → smoke Финансист под boss на проде.
...
| Блок | Статус |
| --- | --- |
| ИП-перевозчик + `no_vat`: закрывающие только акт (без СФ) | ✅ PHP+JS+грид+тесты |
| УУ: матч «весь открытый долг = сумма» + split prefill | ✅ |
| Груз в мастере: нормализация имён / payload | ✅ |
| **Прод** `git pull` + `npm run build` + `optimize:clear` | → в этом деплое |

**Следующий шаг:** smoke — заказ с ИП/без НДС (чек-лист акт); УУ разнесение split.

---

**Обновлено (архив):** 2026-09-16 15:30 · **Ветка:** `master` · **тема:** УУ — ММК 165к split + матч «весь долг»

### Итог сессии 2026-09-16 — платёж ММК 165 000 (сч. 8,9,10)

| Блок | Статус |
| --- | --- |
| Прод: строка #768 → split 32867/32869/32871 (65+65+35) | ✅ `operational_split`, графики paid |
| Матчер: пакет «весь открытый долг контрагента = сумма» | ✅ + авторазнос split + UI prefill |
| PHPUnit `test_suggests_split_when_payment_closes_entire_contractor_open_debt` | ✅ |
| **Прод** PHP-сервисы (pscp) + optimize:clear | ✅ |
| **Не в git / UI build** | код локально; `Reconcile.vue` prefill нужен `npm run build` на проде |

**Следующий шаг:** commit + push при ОТДАТЬ; на проде `npm run build` если нужен prefill «Несколько заявок».

---

**Обновлено (архив):** 2026-09-08 15:20 · **Ветка:** `master` · **тема:** ЭТрН → авто-ЭДО на карточке заказа
...
| Блок | Статус |
| --- | --- |
| Грид `/epd` + sync + связать | ✅ прод `bc2fe06` |
| ЭДО: fallback `ИдентификаторСвязи` | ✅ в этом ОТДАТЬ |
| Table presets: drop removed cols / salary_paid | ✅ в этом ОТДАТЬ |
| Карточка OneC BP + handoff + memento + Я.Диск | ✅ |
| **Прод** хвостов | вместе с ОТДАТЬ |

**Следующий шаг на другом ПК:** `ЗАБРАТЬ` → smoke `/epd`. Далее: OData ЭПЭ/ЭЭР или HTTP fill груза.

**Живые данные 1С:** НОВАФАРМ = ЭПЭ+ЭЭР; АГРО-ПОВОЛЖЬЕ = ошибочный ЭЗЗ («заказ перевозчику, а не поручение»).

---

**Обновлено (архив):** 2026-09-07 19:35 (деплой) · **Ветка:** `master` · **HEAD:** `bc2fe063` · **тема:** модуль ЭПД

### Итог (архив) — модуль ЭПД деплой

| Блок | Статус |
| --- | --- |
| Таблица + sync + грид + связать | ✅ `bc2fe06` |
| migrate + 12 строк реестра + build | ✅ |
...
| Блок | Статус |
| --- | --- |
| `salary:clean-start --date=2026-08-16` — закрыть периоды/хвосты до cutoff | ✅ `b3b6b91f` + применено на прод |
| Прод: 7 периодов → `closed`, 66 начислений, write-off **1 119 294.80 ₽** на дату 16.08 | ✅ |
| Зелинская А.: убрана выплата **24 472.60** из авг h2 (03.09); хвост ушёл в write-off 16.08 | ✅ данные на прод (не в git) |
| Модалка контрагента: ложный «несохранённые изменения» при закрытии без правок | ✅ `2fc57973` на прод + `npm run build` |
| Ранее в сессии: цвет иконок меню / tooltip свёрнутого сайдбара; подписи лида «Суть лида» / «Описание лида» | ⚠ лид-подписи могли остаться только локально — проверить `Wizard.vue` при необходимости |

**Прод:** HEAD `2fc57973` · clean-start уже выполнен · фронт собран.

```text
git pull
pwsh -File scripts/sync-docs-to-yandex.ps1
# при правках Vue: npm run build
```

**Следующий шаг (другой ПК):** ЗАБРАТЬ.

**Отложено:** дробление Wizard/Print; дальше `DB::` в Task/KPI/SalesAssistant; фаза 3 уведомлений реестра; проверить деплой переименований полей лида если нужны на проде.

---

**Обновлено (архив):** 2026-09-02 19:50 (ОТДАТЬ + деплой) · **Ветка:** `master` · **HEAD:** `84e40579` · **тема:** архитектура — ЭДО lookup + вынос `DB::` из грид-контроллеров
...
| Ранее на прод: площадка между машинами, захват мышью, негабарит, «Своя компания» | ✅ |
| **Прод задеплоен** `b4a95e32` · `git pull` + `npm run build` + `optimize:clear` | ✅ |
...
| Блок | Статус |
| --- | --- |
| Колонка «Форма оплаты» в графике: width id 56 vs min:60 (save 422), новая колонка в конец грида | ✅ `929dee07` на прод |
| Баг: каждое сохранение заказа сжимало счётчик нумератора (company_code через `generate`) | ✅ `f0ce35c3` на прод |
| Гонка двух мастеров → один номер (превью писалось как есть): 171/172 | ✅ `1946635a` — create всегда `allocateForCreate` под lock; превью игнорируется |
| АС-ТД-945 «пропал» | ❌ в БД не было; превью после сгоревших номеров |
| Заказ 172: снят факт погрузки, статус → `new` | ✅ на прод вручную |
| Документы ТН↔ЭТрН (ранее в сессии/ветке) | ✅ уже на прод (`15d7877b` / `29a3fa62`) |

**Прод:** `1946635a` + `npm run build`. Миграций нет.
...
**Следующий шаг (другой ПК):** ЗАБРАТЬ. Локально остаются **незакоммиченные** правки document upload / lead-user-guide / Tasks — не входили в этот ОТДАТЬ.

**Отложено / внимание:** уникальный индекс на `orders.order_number` не ставили (soft-delete); ручной номер — только с `order_number_is_manual`.

---

**Обновлено (архив):** 2026-08-17 20:05 (ОТДАТЬ) · **Ветка:** `master` · **HEAD:** `600918ab` · **тема:** инструкция по лидам для менеджера

### Итог сессии 2026-08-17 — лиды, инструкция

| Блок | Статус |
| --- | --- |
| `docs/lead-user-guide.md` — переписана с упором на выгоду менеджера + актуальный UI | ✅ `878e92a6` |
| `docs/leads-mechanism.md` — «Что дальше», котировки, голос, знакомство, конвертация только `transport-intake` | ✅ |
| `docs/crm-user-guide.md` §5 — согласован с новой инструкцией | ✅ |
| Скрипт синхрона Книги: `use Kernel` до bootstrap | ✅ `600918ab` |
| Книга продаж локально (статья id=9 по title) | ✅ |
| Книга продаж **prod** (id=19) | ✅ статья обновлена; прод на `600918ab` |
...
**Следующий шаг (другой ПК):** ЗАБРАТЬ. На проде полный CRM-гайд в Книге (ids 256–263) не пересобирали — обновлена только статья «Лиды — инструкция для пользователя».

---

**Обновлено (архив):** 2026-08-16 21:00 (ОТДАТЬ) · **Ветка:** `master` · **HEAD:** `20578cc4` · **тема:** проверка живого обмена ЭТрН/ЭПД в 1С

### Итог сессии 2026-08-16 — ЭПД: схема vs обмен

| Блок | Статус |
| --- | --- |
| Подрядчики 1С: «обмен ЭТрН уже реализован» | ✅ подтверждено данными OData |
| Не только EntitySet: документы ЭТрН, `РеестрЭПД`, версии титулов, токен авторизации | ✅ `probe-one-c-epd-exchange-evidence.php` |
| Пример ЭТрН в ИБ | № `00000000004` от `2026-07-30` |
| ЭР как Document_* в публикации | ❌ по-прежнему нет |
| Живой POST болванки из CRM | ⏳ не делали |

**Доки:** `docs/one-c-epd-stub-design.md` (раздел «Доказательства живого обмена»).

```text
git pull
```

**Следующий шаг:** у 1С — где ЭР и можно ли OData POST ЭТрН; затем пробный create из CRM на тестовом заказе.

---

**Обновлено (архив):** 2026-08-14 15:16 (ОТДАТЬ) · **Ветка:** `master` · **HEAD:** `118ecfe5` · **тема:** рейсы (км/затраты) + разнос batch + статусы заказов

### Итог сессии 2026-08-14

| Блок | Статус |
| --- | --- |
| Кракозябры в статусе заказа (UTF-8 в `OrderWizardPagePresenter`) | ✅ `5129e26a` |
| ЭР убрана из слотов обязательных документов закрытия сделки | ✅ `5129e26a` |
| Разнос: кнопка «Разнести готовые (N)» (dirty + batch allocate) | ✅ в `81644bee` |
| Рейсы: не сохранялись км/топливо/ЗП (валидация без `order_id`) | ✅ `118ecfe5` |
| Prod deploy `118ecfe5` + frontend build | ✅ |
...
**Операционка:** по заказу **АС-ПВ-771** (рейс `#18`) данные не лежали в БД — после деплоя ввести снова: км **550**, топливо **14932**, ЗП водителя **6600** → «Сохранить».

**Разнос:** массовая кнопка берёт только **вручную изменённые** валидные строки; правило обучается в batch только при явном «Уточнить ключевое слово».

**Отложено:** `ASYNC_OUTBOUND_MAIL=true`; HMAC 1С; живой POST ЭТрН; пароли почты у пользователей (см. архив 13.08).

---

**Обновлено (архив):** 2026-08-13 15:50 (ОТДАТЬ) · **Ветка:** `master` · **HEAD:** `2aca9009` · **тема:** пароль почты ≠ пароль CRM
