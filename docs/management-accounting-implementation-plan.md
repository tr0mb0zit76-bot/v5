...
| M5.2 | `BudgetVarianceService` + UI отклонений | 🟡 черновик аналитики на Index (`ManagementAccountingAnalyticsService`) |
...
| M4.3 | Split: несколько переводов на одну заявку с UI распределения сумм | backlog |
| M4.4 | Ручные операции (наличные) — отдельная форма на Index | backlog (`storeManual` в API) |
| M4.5 | CNY: курс на дату операции | backlog |
| M4.6 | Отчёт «полная картина»: операционный + управленческий за период | backlog |
...
*Версия плана: 1.2 (2026-06-11). M0–M3: `3e622ca`; вкладки и аналитика: `5431a2c`; статьи + MCP: текущий master после push.*
