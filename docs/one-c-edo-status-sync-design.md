...
Фильтр OData по регистру `ОбъектУчета eq '{guid}'` на ИБ Автоальянс **молча пустой** (поле неограниченной длины; `guid'…'` падает). Рабочий lookup: `Document_ЭлектронныйДокументИсходящийЭДО` с `ИдентификаторСвязи eq '{guid-реализации}'` (fallback в `OneCBpClient::findEdoLinksForAccountingObject`).
