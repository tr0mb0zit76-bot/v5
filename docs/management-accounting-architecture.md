...
После **каждого ручного** разноса CRM сама извлекает keyword из назначения (или берёт уточнённый) и upsert’ит `management_reconcile_rules`. Явная галочка больше не обязательна. Авторазнос из pull 1С правила не пишет. При наличии токена `CRM:…` обучение пропускается.
