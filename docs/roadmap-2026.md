...
## Печатные формы — QR + DocMDP (эксперимент) 🧪
...
Документация: [`print-form-pdf-protection.md`](./print-form-pdf-protection.md).

---

## Печатные формы — несколько точек выгрузки у перевозчика 🟡 P2

**Проблема:** если у одного перевозчика на маршруте **несколько точек выгрузки**, в печатную форму (заявка перевозчику) попадает только **последняя** точка — остальные теряются.

**Ожидаемое поведение:** в DOCX должны попадать **все** точки выгрузки этого перевозчика (адреса, города, получатели, даты/время — по согласованному формату шаблона), с учётом контекста плеча / `carrier_slot`, а не одна «последняя» точка заказа целиком.

### Задачи

- [ ] Диагностика: какие плейсхолдеры и legacy-алиасы сейчас мапятся на `unloading_last_*` / `resolvePrimary*` вместо списков (`unloading_addresses`, `route_row_unloading_addresses`, `cargo_recipient.all_addresses`).
- [ ] Исправить снимок в `OrderPrintFormDraftService` для заявки перевозчику: точки маршрута фильтровать по плечу и перевозчику, агрегировать все выгрузки.
- [ ] Проверить шаблоны DOCX перевозчика: при необходимости — таблица маршрута (`route_row_*`) или макросы со списком через `;`.
- [ ] Feature-тест: заказ с 2+ выгрузками на одном плече → черновик содержит обе точки.

*Связано:* `OrderPrintFormDraftService`, `PrintFormPlaceholderPathResolver`, `PrintFormVariableCatalog`, вкладка «Маршрут» (несколько точек на плече).
...
| v0.3.2 | 2026-06-13 | Финансы: частичные оплаты в графике и разнесении, входящие (сумма/кандидаты), наличка → срок от выгрузки; QR/DocMDP — снят appearance подписи |
| v0.3.3 | 2026-06-15 | Бэклог: несколько точек выгрузки у перевозчика — в печатную форму попадает только последняя |
| v0.3.4 | 2026-06-16 | Playbook бизнес-процессов, избранное меню, дашборд по подразделениям; чек-лист документов: ТСД одной группой, наличные без закрывающих |
| v0.3.5 | 2026-06-18 | Управленка: снапшоты бюджета, план/факт (variance), ручные операции, split разнесения; см. `management-accounting-budgeting-integration.md` |
