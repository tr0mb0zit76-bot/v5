...
> **Отложено (SaaS):** вывод CRM в аренду — см. [`saas-roadmap.md`](./saas-roadmap.md) (черновик, 2026-07-09). Не смешивать с текущими P0–P4 без отдельного решения.
...
- [x] Шаринг в UI: роли «Администратор», «Руководитель» → пользователи / группы / workspace (API готов)
...
- [x] ~~UI «сверить хеш»~~ — **не делаем** (бумажная подпись; QR verify для digital PDF достаточно)

Документация: [`print-form-pdf-protection.md`](./print-form-pdf-protection.md).

---

## Печатные формы — несколько точек выгрузки у перевозчика ✅ (2026-06-21)

**Решено:** все точки выгрузки плеча в снимке (`unloading_addresses`, `route_row_*`, фильтр `carrier_slot`).

- [x] `OrderPrintFormDraftService` + тесты

---

## Модули — HTML-конструктор КП из лида 🟡 P3

**ТЗ:** [`tz-step-04-html-proposal-builder.md`](./tz-step-04-html-proposal-builder.md) · **Заменяет** «Сколько влезет → печать».

---

## ~~Модули — «Сколько влезет» + печатные формы~~ ⏸

Заменено HTML-конструктором КП.
...
## Продуктовые модули после ядра (2026-07-20)

Канон: [`product-modules-roadmap.md`](./product-modules-roadmap.md).

| Приоритет | Модуль |
|-----------|--------|
| **Сейчас** | Претензии (Claims) — Юрик |
| Скоро | RFQ/котировки · Playbooks-каталог · Портал заказчика |
| Средний | SLA аккаунта · аттестация менеджера |
| Долгий ящик | Допуск перевозчика · слоты погрузки |
