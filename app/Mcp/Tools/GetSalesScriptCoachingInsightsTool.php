...
#[Description('Аналитика живых прохождений скриптов: исходы, возражения, слабые менеджеры, проблемные сценарии и рекомендации руководителю. Дополняет get_trainer_coaching_insights (тренажёр).')]
class GetSalesScriptCoachingInsightsTool extends Tool
{
    use LogsMcpToolCalls;

    public function __construct(
        private readonly SalesScriptCoachingInsightsService $insights,
    ) {}
    ...
    public function handle(Request $request): Response
    {
        return $this->withMcpAccess($request, function (User $user) use ($request): Response {
            if (! RoleAccess::canViewTrainerAnalytics($user) && ! RoleAccess::canViewAiAnalytics($user)) {
                throw new AuthenticationException('Нет доступа к аналитике скриптов.');
            }
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            ...
            'sample_limit' => $schema->integer()
                ->description('Сколько проблемных сессий вернуть в выборке (5–50).')
                ->min(5)
                ->max(50),
        ];
    }
