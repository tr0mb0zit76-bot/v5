...
final class TrainerClientReactionMatcher
{
    ...
    private function keywordScore(string $reply, string $reactionKey): int
    {
        return match ($reactionKey) {
            ...
            'competitor' => $this->containsAny($reply, ['конкурент', 'другой подрядчик', 'уже работаем', 'текущий перевозчик', 'другая компания']) ? 5 : 0,
            'need_info' => $this->containsAny($reply, ['пришлите', 'на почту', 'документ', 'кп', 'расчёт', 'подробн', 'уточн']) ? 4 : 0,
            'stall' => $this->containsAny($reply, ['не сейчас', 'позже', 'занят', 'перезвон', 'неудобно', 'нет времени']) ? 4 : 0,
            default => 0,
        };
    }
